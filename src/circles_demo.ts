...
const settings: SystemSettings = {
    ...
    speed: 2,
    velocity: (state: ParticleState) => {
        const dist = Math.sqrt(
            (cursorPosition.x - state.x) ** 2 +
                (cursorPosition.y - state.y) ** 2
        );

        // accelerate particles laterally relative to cursor

        const { x: mx, y: my } = cursorPosition;
        const dy = state.y - my;
        const dx = state.x - mx;
        const dirToPoint = Math.atan2(dy, dx);

        const lateralDir = dirToPoint + Math.PI / 2;

        const dirTo = lateralDir + Math.PI;

        // const accel = Math.max(Math.min(750 / dist, 8), 3);

        const inSpinRange = dy ** 2 + dx ** 2 < 100 ** 2;

        const accel = inSpinRange ? 100 / dist : 0.01;
        ...
        if (inSpinRange) {
            return {
                vy:
                    dragFac *
                    (Math.sin(lateralDir) * accel +
                        Math.sin(state.direction) * state.speed),
                vx:
                    dragFac *
                    (Math.cos(lateralDir) * accel +
            ...
            return {
                vy:
                    dragFac *
                    (Math.sin(dirTo) * accel +
                        Math.sin(state.direction) * state.speed),
                vx:
                    dragFac *
                    (Math.cos(dirTo) * accel +
                        Math.cos(state.direction) * state.speed),
            };
        }
